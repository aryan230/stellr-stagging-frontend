...
export default class MentionTool {
  constructor({ holder, allUsers, baseUrl, accessKey = "@", searchAPIUrl }) {
    /*
     ...
     */
    this.allUserListItemsCache = this.cacheAllUsersAsUserListItems(
      this.allUsers,
      this.baseUrl,
      this.accessKey
    );
     ...
     */
    this.initialUserlistItemsOrder = this.createAllUserListItems(
      this.allUsers,
      this.baseUrl,
      this.accessKey
    );
     ...
     */
    this.nodes.userMentionToolbar = this.createUserMentionToolbar(
      this.nodes.searchBar,
      this.nodes.usersList
    );
  ...
  hideUserMentionToolbarAndChangeFocus(holder, accessKey) {
    ...
    const eventListner = function(e) {
       ...
       */
      classObj.prevCaretPosAndSelectedNode = classObj.getCaretPositionAndSelectedNode(
        classObj.prevActiveElement
      );
    ...
    document.getElementById(holder).addEventListener("focusin", function() {
       ...
       */
      if (
        classObj.nodes.userMentionToolbar != document.activeElement &&
        !classObj.nodes.userMentionToolbar.contains(document.activeElement)
      ) {
        /**
        ...
        if (classObj.nodes.userMentionToolbar.style.display != "none") {
          ...
          if (
            classObj.prevActiveElement &&
            classObj.prevActiveElement != null
          ) {
    ...
    async function searchQueryListner() {
      if (classObj.searchAPIUrl) {
        try {
           ...
           */
          const userListItems = classObj.createAllUserListItems(
            usersBasedOnSearchQuery,
            "",
            accessKey
          );
           ...
           */
          classObj.nodes.userMentionToolbar.lastChild = classObj.createUsersList(
            userListItems
          );
        } catch (error) {
          console.log(error);
  ...
  createUserListItem({
     ...
     */
    const userMetadataContainer = create(
      "div",
      [this.CSS.userMetadataContainer],
      {},
      [userFullNameContainer, userSlugContainer]
    );
     ...
     */
    const userProfileContainer = create(
      "div",
     ...
     */
    const userListItemWrapper = create(
      "div",
      [this.CSS.userListItemWrapper],
      {},
      [userProfileContainer, userMetadataContainer]
    );
      ...
      const userMentionLink = create(
        ...
        [
          document.createTextNode(
            keysEnum[accessKey]["createTextNode"](userFullName)
          ),
        ]
      );
