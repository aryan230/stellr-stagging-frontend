...
function DrawerEditTask({
  ...
  const { task: updatedTask, sucess } = taskUpdate;

  const submitHandler = async (e) => {
    e.preventDefault();
    let subjectEdited = task.subject == subject ? false : true;
    let statusEdited = task.status == status.value ? false : true;
    let priorityEdited = task.priority == priority.value ? false : true;
    let dueDateEdited = task.due_date == dueDate ? false : true;
    ...
    const logObject = {
      ...
      message: `Updated the task with ${subjectEdited ? `subject` : ``} ${
        statusEdited ? `status` : ``
      } ${priorityEdited ? `priority` : ``}  ${
        dueDateEdited ? `due date` : ``
      }`,
    };
    await dispatch(
      updateTask(task._id, {
    ...
    await dispatch({ type: TASK_UPDATE_PROFILE_RESET });
    setEditTask(false);
  };
  ...
  return (
    <EmptySlideOvers open={editTask} setOpen={setEditTask}>
        ...
        <div className="drawer-info-inside">
          <div className="drawer-info-top">
            ...
            <button
              ...
              className="text-white w-full bg-indigo-700 hover:bg-indigo-800 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-4 mr-2 mb-2 focus:outline-none"
            >
              Save
            </button>
          </div>
